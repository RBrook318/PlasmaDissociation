...
integer ::  nc, mult, nfr, nfr2, natomf
...
do i=1,natom-1
do j=i+1,natom
val=sqrt(sum((f0(3*i-2:3*i)+f0(3*j-2:3*j))**2))
if(val.lt.1.e-5) then
   nfr=i
   nfr2=j
   if (atom(i).eq.'C'.or.atom(j).eq.'C') then
   mult=mult-1
   if (mult==2) mult=4
   end if
   natomf=natom-2
...
write(2,'(2i3)')nc,mult
do i=1,natom
  if (i.ne.nfr.and.i.ne.nfr2) write(2,'(a1,3f22.16)') atom(i),r1(i*3-2:i*3)
end do
write(2,*)
do i=1,natom
  if (i.ne.nfr.and.i.ne.nfr2) write(2,'(3e25.16)') p1(i*3-2:i*3)
end do
...
do i=1,nst
  n=0
  do j=1,ndim
    if(j==nfr*3.or.j==nfr*3-1.or.j==nfr*3-2.or.j==nfr2*3.or.j==nfr2*3-1.or.j==nfr2*3-2) cycle
    n=n+1
